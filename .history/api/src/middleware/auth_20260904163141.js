...
if (keycloakConfig.publicKey) {
  ...
  console.log(
    "[Auth Init] Mode Cle Publique Statique active pour la verification des tokens.",
  );
} else {
  console.log(
    `[Auth Init] URL JWKS configuree pour le backend : ${keycloakConfig.jwksUri}`,
  );
}

const client = keycloakConfig.publicKey
  ? null
  : jwksClient({
      ...
      handleSigningKeyError: (err) => {
        console.error(
          "❌ [JWKS-RSA] Erreur lors de la recuperation de la cle de signature:",
          err.message,
        );
      },
    });
...
function getKey(header, callback) {
  ...
  if (!header || !header.kid) {
    console.error(
      "❌ [Auth Middleware] Le token JWT ne contient pas de 'kid' dans son en-tete.",
    );
    return callback(
      new Error(
        "Le token JWT ne contient pas de parametre 'kid' dans son en-tete.",
      ),
    );
  }
  ...
  client.getSigningKey(header.kid, (err, key) => {
    if (err) {
      console.error(
        `❌ [JWKS-RSA] Echec de la recuperation de la cle pour le kid "${header.kid}":`,
        err.message,
      );
      return callback(err);
    }
...
module.exports = async (req, res, next) => {
  ...
  try {
    ...
    if (impersonation && impUserId) {
      const isAdmin =
        req.user.roles.includes("admin_cabinet") ||
        req.user.roles.includes("commercial_cabinet");
      if (isAdmin) {
        // Vérifier si l'admin/commercial est explicitement autorisé à simuler cet utilisateur
        const isAuthorized = await authService.checkSimulationPermission(
          req.user.id,
          impUserId,
        );
        if (!isAuthorized) {
          console.warn(
            `[Impersonation Blocked] Admin/Commercial user "${req.user.id}" attempted unauthorized simulation of user "${impUserId}"`,
          );
        ...
        if (impUser) {
          ...
          if (impUser.Id_Auth && impUser.Id_Auth.trim()) {
            try {
              ...
              console.error(
                "Echec de la recuperation des roles de l'utilisateur simule:",
                e.message,
              );
            }
          } else {
            ...
            req.user.roles =
              impUser.Mobile === "O" || impUser.mobile === "O"
                ? ["adherent"]
                : ["client"];
          }
          ...
          try {
            ...
            console.error(
              "❌ Echec de la mise a jour du token de l'utilisateur simule en BDD:",
              err.message,
            );
          }
        }
    ...
    if (dbUser && dbUser.token !== token && !impersonation) {
      try {
        ...
        console.error(
          "❌ Echec de la mise a jour du token en BDD:",
          err.message,
        );
      }
    }
    ...
    if (siteIdHeader) {
      ...
      const needsSite = !bypassRoutes.some((route) =>
    ...
    console.error(
      "❌ [Auth Middleware] Echec de validation du token:",
      err.message,
    );
...
module.exports.checkRole = (roles) => {
  return (req, res, next) => {
    ...
    if (req.adminUser) {
      ...
      if (
        adminRoles.includes("admin_cabinet") ||
        adminRoles.includes("commercial_cabinet")
      ) {
        return next();
      }
    }
    ...
    if (!hasRole)
      return error(res, "Acces refuse : permissions insuffisantes.", 403);
    next();
  };
};
